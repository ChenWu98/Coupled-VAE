...
class Decoder(nn.Module):
    ...
    def forward(self, helper, init_input, test_lm=False):
        """
        :param helper: shape = (n_batch, max_len)
        :param init_input: shape = (n_batch, emb_dim)
        ...
        if self.training or test_lm:
            ...
            cond = init_input.unsqueeze(1).expand(-1, T, -1)  # shape = (n_batch, max_len, emb_dim)
            helper_emb = torch.cat([helper_emb, cond], dim=2)  # shape = (n_batch, max_len, emb_dim * 2)

            init_input = init_input.unsqueeze(1)  # shape = (n_batch, 1, emb_dim)
            ...
            init_input = init_input.unsqueeze(1)  # shape = (BN, 1, emb_dim)
...
class CNN_VAE_COUPLE(nn.Module):
    ...
    def __init__(self, hid_dim, latent_dim, enc_layers, dec_layers, dropout, enc_bi, dec_max_len, beam_size, WEAtt_type, encoder_emb, decoder_emb, pad_id,
                 scale):
        super(CNN_VAE_COUPLE, self).__init__()
        ...
        self.DecoderCouple = Decoder(scale=scale,
                                         voc_size=self.voc_size,
                                         latent_dim=self.latent_dim,
                                         emb_dim=self.emb_dim,
                                         dropout=self.dropout,
    ...
    def importance_sampling(self, gaussian_dist, go, eos, n_sample):
        ...
        for sample_id in range(n_sample // _n_sample):
            ...
            _logits = self.Decoder(init_input=_init_input.view(_n_sample * B, self.emb_dim),  # shape = (_n_sample * n_batch, out_dim)
    ...
    def decode_from(self, latents, go):
        ...
        return self.Decoder(init_input=init_input,
                            helper=go)
    ...
    def forward(self, go, sent_len=None, bare=None):
        ...
        if not self.training:
            ...
            logits = self.Decoder(init_input=init_input,
                                  helper=go)
            logits_couple = self.DecoderCouple(init_input=init_input_couple,
                                               helper=go)
